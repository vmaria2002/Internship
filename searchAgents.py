...
#Pentru problema corner, este nevoie sa suprascriem metodele:  def getStartState(self)..
    ...
    def __init__(self, startingGameState):
        ...
        for corner in self.corners:
            if not startingGameState.hasFood(*corner):
                print('Warning: no food in corner ' + str(corner)) #nu avem mancare in coltul la care e trimite algoritmul
...
#aici am modificat
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        colturiVizitate = state[1] #cum am mai sus:AllCornersVisited
        #suntem in finish, doar daca  taote colturile sunt True: vizitate
...
#aici am modificat
    def getSuccessors(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            pozitie, colturiVizitate = state #(pos, listaColturi)
            x, y = pozitie
            dx, dy = Actions.directionToVector(action) #gasim calea pe care o urmeaza in dunctie de actiunea pe care o are N, S..
            nextx, nexty = int(x + dx), int(y + dy) # updatam distanta parcursa
            hitsWall = self.walls[nextx][nexty] #vedem daca avem zid in locul unde vrem sa mergem
            nextState = (nextx, nexty) #starea la care vrm sa mutam
            if not hitsWall: #daca nu e zid
                if nextState in self.corners:  #luam toate coltutile
                    #cazuri pt toate cele 4 colturi
                    ...
                    #daca nu e in niciun colt, atunci, vom inainte o pozitie fata de directia in care ma aflu
                    successor = ((nextState, colturiVizitate), action, 1)
                #adaugam succesorul cand ajungem la colt
                children.append(successor)

        self._expanded += 1  # numarul de nofuri expandate creste
        return children
